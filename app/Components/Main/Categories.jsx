...
const Categories = () => {
  return (
    <div className="py-16 text-grey">
      <div className={`${salsa.className} flex items-center justify-between`}>
        <div className="h-[1px] bg-black w-[36.5vw] rounded-full relative">
      ...
      <div
        className={`grid grid-cols-4 gap-x-8 gap-y-8 mt-4 px-5 pt-3 ${sansation.className}`}
      >
        {[
...
const Block = ({ data }) => {
  return (
    <div className="flex flex-col items-center relative cursor-pointer rounded-md transition-all hover:scale-105">
      <Image src={data?.image} alt="Img" className="w-full h-full object-cover object-center rounded-md transition-all" />
      <h1 className="text-2xl font-medium text-white absolute mt-0 pt-0 text-center bottom-10 left-1/2 -translate-x-1/2">
        {data?.title}
      </h1>
    </div>
  );
